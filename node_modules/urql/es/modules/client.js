...
import { hashString } from "./hash"; // Response from executeQuery call
...
function () {
  ...
  _proto.executeQuery = function executeQuery(queryObject, skipCache) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      var hash = hashString(body); // Check cache for hash

      _this2.cache.read(hash).then(function (data) {
        if (data && !skipCache) {
          var _typeNames = gankTypeNamesFromResponse(data);
          ...
          fetch(_this2.url, _extends({
            ...
            if (response.data) {
              ...
              var _typeNames2 = gankTypeNamesFromResponse(response.data); // Store data in cache, using serialized query as key
    ...
    return new Promise(function (resolve, reject) {
      ...
      fetch(_this3.url, _extends({
        ...
        if (response.data) {
          ...
          var _typeNames3 = gankTypeNamesFromResponse(response.data); // Notify subscribed Connect wrappers
